...
| 9 — Réversibilité des migrations en CI | en cours | 0 | Job CI `reversibilite` ajouté : cluster vierge dédié (DROP ROLE est global au cluster), up→down→diff avec l'état vierge→re-up→diff (schéma pg_dump + rôles + extensions + ACL de schéma). Poussé, verdict CI attendu |
