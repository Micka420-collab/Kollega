...
`cargo test --workspace` : **209 tests, 0 échec** en local, et **exécutés en CI sur un PostgreSQL réel** (GitHub Actions, service pgvector/pg16).
...
**Toute preuve existante a été vue ÉCHOUER, volontairement.** Un test vert ne prouve rien tant qu'on ne l'a pas vu rouge : il peut n'avoir jamais rien vérifié. Chaque invariant marqué prouvé a donc été falsifié exprès pour voir si sa preuve le remarquait — refus par défaut inversé en autorisation, contenu externe concaténé à l'instruction, rechargement du solde retiré, `REVOKE DELETE` ôté d'une migration, `tokio` introduit dans le domaine. Toutes ont rougi, et le détail est dans la [rubrique « Sensibilité » de la matrice](docs/matrice-invariants.md). Un piège y est consigné plutôt que tu : un premier essai a conclu à tort qu'une preuve était insensible, alors que la CI s'était arrêtée avant de l'exécuter — un test qui n'a pas tourné ressemble en tout point à un test qui n'a rien vu. La CI passe donc en `--no-fail-fast`, non par confort de diagnostic, mais parce que sans cela un sabotage peut mentir dans le sens rassurant. Et une limite de la méthode y est écrite noir sur blanc : **sept des dix garde-fous du dépôt avaient au moins un trou — dix trous en tout — alors qu'ils avaient tous été « vérifiés par sabotage »** ; chacun l'avait été sous la forme exacte qu'il savait reconnaître. Un sabotage écrit par l'auteur de la garde teste l'intention, pas la couverture. Le plus grave était ancien et touchait l'invariant 1 : trois écritures équivalentes (`FALSE`, `'f'`, un argument paramétré) posaient le contexte d'organisation en portée **session**, où il survit au retour de la connexion dans le pool — la requête suivante, celle d'un autre client, se serait exécutée sous le contexte du précédent.
...
- Aucun binaire qui tourne en continu, au sens d'un service exploité — rien n'est déployé nulle part. Le serveur, lui, **démarre, sert et s'arrête proprement** : `kollega serve` se connecte à PostgreSQL, répond sur `GET /health`, et rend la main sur `SIGTERM` ou `Ctrl-C` au lieu d'être tué au milieu des requêtes en vol (prouvé par un test qui l'écoute vraiment, sans base requise). Depuis le 29/07 il porte aussi **une écriture et une lecture réelles** de tâche, à travers `Db::org_transaction` : créer puis relire une tâche par HTTP fonctionne, et la tâche d'une autre organisation rend `404` — indiscernable d'une tâche inexistante, donc inénumérable.
- **Ce que le serveur n'a toujours pas** : aucune authentification de requête, aucune session. L'organisation vient donc du CHEMIN de l'URL, ce qui est un **échafaudage explicitement inapte à la production** — la RLS empêche un client de lire les données d'un autre, pas d'écrire chez lui. Inscrit à [`docs/backlog.md`](docs/backlog.md), à remplacer par l'organisation issue de la session (M1). Aucune route ne fait avancer une tâche : cela suppose de brancher un `ModelProvider`, décision ouverte. Pas d'appel de modèle, pas de client MCP.
