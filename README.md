...
**Dix des treize preuves ont été vues ÉCHOUER, volontairement.** Un test vert ne prouve rien tant qu'on ne l'a pas vu rouge : il peut n'avoir jamais rien vérifié. Chaque invariant marqué prouvé a donc été falsifié exprès pour voir si sa preuve le remarquait — refus par défaut inversé en autorisation, contenu externe concaténé à l'instruction, rechargement du solde retiré, `REVOKE DELETE` ôté d'une migration, `tokio` introduit dans le domaine. Toutes ont rougi, et le détail est dans la [rubrique « Sensibilité » de la matrice](docs/matrice-invariants.md) — y compris ce qui n'a **pas** encore été falsifié.
